...
locals {
  ...
  default_firewall_rules = {
    "allow-internal" = {
      ...
      description   = "Allow internal traffic between instances in the spoke network"
      direction     = "INGRESS"
      priority      = 1000
...
resource "google_compute_network" "spoke_network" {
  ...
  mtu                     = var.mtu
  description             = "Spoke VPC network for Hub and Spoke architecture"
...
resource "google_compute_subnetwork" "subnets" {
  ...
  network       = google_compute_network.spoke_network.id
...
resource "google_compute_router" "nat_router" {
  ...
  region      = each.value
  network     = google_compute_network.spoke_network.id
  description = "Router for Cloud NAT in the spoke network"
}
...
resource "google_compute_address" "nat_address" {
  ...
  description  = "External IP address for Cloud NAT in the spoke network"
}
